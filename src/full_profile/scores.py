...
class scores:
    ...
    def _reconstruction_test(self, i):
        col = i
        row = self.reference.indices[self.reference.indptr[i]:self.reference.indptr[i+1]]     
        return self.factored[0][row, :]@self.factored[1][:, col]-self.reference.data[self.reference.indptr[i]:self.reference.indptr[i+1]] 
    ...
    def reconstruction_score_peak_picking_match(self) -> float:
        ...
        return self.reconstruction_score_peak_picking(int(compressed_size/self.reference.shape[1])), int(compressed_size/self.reference.shape[1])