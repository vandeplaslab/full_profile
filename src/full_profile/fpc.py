...
class fpc:
    ...
    def b(self) -> ty.Array:
        ...
        """
        return matmul(multiply(np.array(self._b[0]), np.array(self._b[1])), np.array(self._b[2]))
    ...
    def _initialize_b(self) -> None:
        ...
        self._b = [0, 0, 0]
        self._b[0] = zeros(
            (self._m, 1), atype=self.btype, dtype=self.datatype
        )
        self._b[1] = zeros((1, 1), atype=self.btype, dtype=self.datatype)
        self._b[2] = zeros(
            (1, self._n), atype=self.btype, dtype=self.datatype
        )
        return None
    ...
    def initialize(self, a: ty.Array) -> None:
        """
        ...
        self.y = zeros(
            (self._m, self._n), atype=self.btype, dtype=self.datatype
        )
        
        #zeros((self._m, self._n), atype=self.atype, dtype=self.datatype)
        self.x_sparse = self._ss.csc_matrix((np.zeros_like(self.a.data), self.a.indices, self.a.indptr), shape=(self._m, self._n))
        #zeros((self._m, self._n), atype=self.atype, dtype=self.datatype)
        self.tau = self.tau_factor * builtins.max(self.a.shape)
    ...
    def stopping_criterion(self) -> bool:
        ...
        if crit < self.stop_criteria:
            if self.verbose:
                print('Iteration ' + str(self.k) + ' - Crit = ' + str(crit) + ' - Rank = ' + str(self._svp) + ' - Time = '+ str(time.time()-self.tic))
            return True
        else:
            if self.verbose:
                print('Iteration ' + str(self.k) + ' - Crit = ' + str(crit) + ' - Rank = ' + str(self._svp) + ' - Time = '+ str(time.time()-self.tic))
    ...
    def update_x_sparse(self) -> None:
        ...
        for i in range(self.a.shape[1]):
            ...
            self.x_sparse.data[self.x_sparse.indptr[i]:self.x_sparse.indptr[i+1]] = uv[row,col] if self.dense else u[row,:]@v[:, col]
