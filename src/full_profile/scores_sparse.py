...
class scores_sparse:
    def __init__(
        ...
        self.reference_data = reference[0]
        self.reference_indices = reference[1]
        self.reference_indptr = reference[2]
        self.reference_shape = reference[3]
...
#         raw_sparse_size = (self.reference.data.nbytes + self.reference.indices.nbytes + self.reference.indptr.nbytes) / (10**9)
...
#             x_sparse.data[x_sparse.indptr[i]:x_sparse.indptr[i+1]] = uv[row,col] if dense else u[row,:]@v[:, col]
...
#         a_fro = linalg.norm(self.reference_data, 2)
...
#         return linalg.norm((self.reference.data - x_sparse_data), 2) / a_fro
    ...
    def _reconstruction(self, i):
        col = i
        row = self.reference_indices[self.reference_indptr[i]:self.reference_indptr[i+1]]     
        return np.linalg.norm(self.factored[0][row, :]@self.factored[1][:, col]-self.reference_data[self.reference_indptr[i]:self.reference_indptr[i+1]], 2)**2
    ...
    def reconstruction_score(self, n_jobs : int = 2) -> float:
        ...
        A = Parallel(n_jobs=n_jobs)( # max 51/52 
            delayed(self._reconstruction)(i) for i in range(self.reference_shape[1])
        )
        
        #A = np.sum(np.array(A))

        return np.array(A), a_fro#np.sqrt(A) / a_fro