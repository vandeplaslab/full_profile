...
class svt:
    ...
    def _initialize_b(self) -> None:
        ...
        self._b = [0, 0, 0]
        self._b[0] = zeros(
            (self._m, 1), atype=self.atype, dtype=self.datatype
        )
        self._b[1] = zeros((1, 1), atype=self.atype, dtype=self.datatype)
        self._b[2] = zeros(
            (1, self._n), atype=self.atype, dtype=self.datatype
        )
        return None
    ...
    def initialize(self, a: ty.Array) -> None:
        ...
        self.y = self._ss.csc_matrix((np.zeros_like(self.a.data), self.a.indices, self.a.indptr), shape=(self._m, self._n))
